...
class MainChatFrame(ctk.CTkFrame):
    def __init__(self, parent, ai_response):
        ...
        self.user_input_entry.place(
            relx=0.49, rely=0.95, anchor=ctk.CENTER, relwidth=0.79)
    ...
    def on_key_release(self, event):
        if len(self.user_input_entry.get()) > 0:
            self.send_button_widget.configure(
...
class UserInputFrame(ctk.CTkFrame):
    def __init__(self, parent, user_input):
        ...
        self.label_user = ctk.CTkLabel(self, text='You', text_color=WHITE, font=(
            FONT_REGULAR, 14)).pack(pady=0, anchor='e', padx=50)
